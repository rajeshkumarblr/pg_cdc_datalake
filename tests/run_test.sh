...
psql -U cdc -d ecommerce -c "SELECT pg_drop_replication_slot('pg_cdc_datalake_slot');" || true

echo "2. Setting up 100 products..."
python3 tests/setup_100_products.py
...
python3 tests/stream_50_products.py
